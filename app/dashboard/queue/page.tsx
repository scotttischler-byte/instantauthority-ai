...
export default function QueuePage() {
  ...
  useEffect(() => {
    void fetch("/api/releases").then((r) => r.json()).then((d) => setRows(d.releases ?? []));
  }, []);
          ...
          <tbody>
            {filtered.map((r) => (
              <tr key={r.id} className="border-t border-electric/10">
                ...
                <td className="space-x-1 py-2"><button className="rounded border px-2 py-1 text-xs">Copy</button><button className="rounded border px-2 py-1 text-xs">Ready</button><button className="rounded border px-2 py-1 text-xs" onClick={() => setShowDistribution(r.id)}>Submitted</button><button className="rounded border px-2 py-1 text-xs">Edit</button><button className="rounded border px-2 py-1 text-xs">Delete</button></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {showDistribution ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-charcoal/50 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-5">
            ...
            <button className="mt-3 rounded bg-cyan px-3 py-2 text-sm font-semibold text-charcoal" onClick={() => setShowDistribution(null)}>Save Distribution</button>
          </div>
        </div>
      ) : null}
    </div>
